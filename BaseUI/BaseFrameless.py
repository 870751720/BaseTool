...
class BaseFramelessWindow(object):
	...
	def nativeEvent(self, eventType, message):
		...
		if msg.message == win32con.WM_NCCALCSIZE:
			...
			if resizeWidth and resizeHeight:
				# bottom left corner
				if winRect.left <= x < winRect.left + borderWidth and winRect.bottom - borderWidth <= y < winRect.bottom:
					return True, win32con.HTBOTTOMLEFT
				# bottom right corner
				if winRect.right - borderWidth <= x < winRect.right and winRect.bottom - borderWidth <= y < winRect.bottom:
					return True, win32con.HTBOTTOMRIGHT
				# top left corner
				if winRect.left <= x < winRect.left + borderWidth and winRect.top <= y < winRect.top + borderWidth:
					return True, win32con.HTTOPLEFT
				# top right corner
				if winRect.right - borderWidth <= x < winRect.right and winRect.top <= y < winRect.top + borderWidth:
	...
	def isCaptionArea(self, x, y):
		return y < self._titleHeight
