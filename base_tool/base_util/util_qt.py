...
def calculate_text_size(text: str, width: int, font: QFont) -> tuple[int, int]:
    """计算文本在指定宽度下的尺寸"""
...
class _DummyQObject(QSingleton):
    ...
    def __init__(self) -> None:
        ...
        self._timers = set()
        self.callInMainThreadSignal.connect(self._on_request_call_in_main_thread)

    def add_timer(self, singleShot=False, startTime=0, timeoutFunc=None) -> QTimer:
        if not singleShot:
            assert startTime > 0
        timer = QTimer(self)
        timer.setSingleShot(singleShot)
        timer.timeout.connect(lambda: self._on_timeout(timer, timeoutFunc))
        timer.start(startTime)
        self._timers.add(timer)
        return timer

    def clear_timer(self, timer) -> None:
        if timer in self._timers:
            self._timers.remove(timer)
            timer.stop()
            timer.deleteLater()

    def _on_timeout(self, timer, func) -> None:
    ...
    @staticmethod
    def _on_request_call_in_main_thread(func, args, kwargs) -> None:
        func(*args, **kwargs)


_dummy_q_object = _DummyQObject()
    ...
    def __call__(self, fn):
        ...
        def decorated(*args, **kwargs):
            ...
            if self._immediate:
                ...
                self._timeout = self._create_timer(
                    lambda: self._later(False), self._wait
                )
                if callNow:
                    self._result = self._fn(*self._args, **self._kwargs)
            else:
                self._timeout = self._create_timer(
                    lambda: self._later(True), self._wait
                )
